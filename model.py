...
from gumball_lite_environ import *
...
class Model(object):
	...
	def __init__(self, policy, ob_space, action_space, nenvs, nsteps, ent_coef, vf_coef, max_grad_norm):
		...
		def neglogp(state, valid_ins, actions):
			return sess.run(neglogpac, {network.X: state, network.available_moves: valid_ins, actions_:actions})
...
def testing(model):
	...
	"""
	test_env = DummyVecEnv([env.make_gumball_env()])
	...
	for trial in range(3):
		...
		while done == False:
			...
			transcript = info[0]['song']
			valid_moves = info[0]['valid_actions']
